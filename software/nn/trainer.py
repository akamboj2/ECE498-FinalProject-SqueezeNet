...
class BackPropTrainer(Trainer):
    ...
    def __eval_single_batch_compute__(self, x, y):
        ...
        input_ = inputs[0][0][0].numpy().astype(float)
        # print(input_.shape)
        ...
        bias_squeeze = self.trainer_configs.model.features[7].squeeze.bias.data.numpy().astype(float)
        # print(bias_squeeze.shape)
        bias_expand1x1 = self.trainer_configs.model.features[7].expand1x1.bias.data.numpy().astype(float)
        # print(bias_expand1x1.shape)
        bias_expand3x3 = self.trainer_configs.model.features[7].expand3x3.bias.data.numpy().astype(float)
        print(bias_expand3x3.shape)
        ...
        file.close()
        file = open("squeeze_bias.txt", "w+")
        bias_squeeze.tofile(file, ",") 
        file.close() 
        file = open("expand1x1_bias.txt", "w+")  
        bias_expand1x1.tofile(file, ",") 
        file.close()
        file = open("expand3x3_bias.txt", "w+")  
        bias_expand3x3.tofile(file, ",") 
        file.close()
        loss = self.trainer_configs.criterion(output, y)
